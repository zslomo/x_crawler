...
search = 'rae-lil-black'
root_page = "https://www.pornhub.com/video/"
start_page = "https://www.pornhub.com/pornstar/{}?page={}".format(search, start_page_num)
dist_root = 'G:\\pornhub\\star\\{}'.format(search)
# socks proxy
...
for i in range(start_page_num, end_page_num):
    ...
    while len(page_list) == 0 and k < 5:
        try:
            print('get page {}, {} times'.format(curr_page, k))
            html = s.get(curr_page, headers=headers).content
            # print(html)
            html = html.decode('utf-8')
            html = etree.HTML(html)
            page_list = html.xpath('//*[@id="pornstarsVideoSection"]/li/div/div[3]/span/a/@href')
            if len(page_list) == 0:
                page_list = html.xpath('//*[@id="claimedUploadedVideoSection"]/li/div/div[3]/span/a/@href')
            # print(page_list)
        except Exception as e:
            print(str(e))
            continue
    ...
    curr_page = "https://www.pornhub.com/pornstar/{}?page={}".format(search, i + 1)
s.close()
